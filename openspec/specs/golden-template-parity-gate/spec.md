# golden-template-parity-gate Specification

## Purpose

Release-build-time check that fails `scripts/build-release.mjs` when
`assets/golden/` content diverges from the `src/project_root_structure/`
sources, preventing a stale or manually-edited golden copy from shipping.
