# golden-template-sync Specification

## Purpose

Build-time step that copies the five golden agent-context files from
`src/project_root_structure/` into
`src/skills/ondemand/ts-project-init-advisor/assets/golden/`, preserving
subpaths, so the advisor skill package carries its own canonical copy
independent of the installer's scaffold path.
