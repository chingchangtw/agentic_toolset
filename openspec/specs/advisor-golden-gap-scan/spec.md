# advisor-golden-gap-scan Specification

## Purpose

ts-project-init-advisor behavior that scans a target project for each of the
five golden files and either creates the file directly (absent case,
guarded) or emits an approval-gated MODIFY recommendation task (present
case, gap analysis vs golden content, never auto-writes).
