...
When `/ts-iteration next` calls `/ts-deliver init`, the `type` field pre-populates
the ts-deliver-router registry with the correct profile. The agent running
`/ts-deliver init` reads this file to configure phases, gates, and tool tiers.
...
## Size Estimate → Iteration Sequencing Guidance

Used by `/ts-iteration start` to warn when an iteration is overloaded.
...
**XL epics:** ts-project-planner will warn and offer to split before `/ts-iteration start`.
Splitting rule: find the natural Bounded Context boundary in `domain.json` and
create two epics with `depends_on` ordering.
