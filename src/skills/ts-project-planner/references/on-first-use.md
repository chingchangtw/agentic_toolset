...
Replace the sub-command line with the appropriate prefix/subcommand, and `<description>`
with the matching text below (source of truth: `references/commands.md`):

**ts-iteration**
- **start** (`/ts-iteration:start <release>`): Loads a release's epics from plan.json,
...
- **next** (`/ts-iteration:next`): Activates the next queued epic and calls
...
- **close** (`/ts-iteration:close <release>`): Closes a release once every epic is done
...
**ts-discover**
- **idea** (`/ts-discover:idea "<desc>"`): Seeds the Discovery backlog with one new
  ...
  ts-deliver-router.
- **explore** (`/ts-discover:explore <id>`): Runs Problem Understanding + Solution
...
- **validate** (`/ts-discover:validate <id>`): Pressure-tests an idea's riskiest
...
- **decide** (`/ts-discover:decide <id> [build|kill|keep-learning|reduce-scope]`):
  Terminal decision point for an idea — promote to ready-for-delivery, kill with a
  written ADR, loop back for more learning, or split into smaller ideas. `build`
  requires a passing DDD validation first.
- **status** (`/ts-discover:status`): Kanban view of the Discovery backlog — counts and
...
**ts-project**
- **plan** (`/ts-project:plan --new "<vision>"` or `--sync [release]`): Two modes —
...
- **status** (`/ts-project:status`): Cross-iteration progress snapshot — releases and
  their state, total epics done vs. total, Discovery pipeline counts, open risks, and
  the most recent ADR.
- **refine** (`/ts-project:refine`): Backlog maintenance after a release closes or scope
