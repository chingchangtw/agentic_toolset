...
Loaded when: reading/verifying/writing `.agents/ts-deliver-router/state.json` (algo steps 0,2,6).

`.agents/ts-deliver-router/state.json` is written by each phase on exit, read by the router every invoke.
...
`.agents/ts-deliver-router/history.jsonl` is an append-only audit log. One JSON object per line. Never read during routine routing (algo step 2). Read only by `/ts-deliver:status --history` and ingest resolution.
...
5. Atomically replace `.agents/ts-deliver-router/state.json` (write tmp → rename). The written file MUST NOT contain `phase_history` or `ingest_log` keys.
6. Append one `phase_exit` event to `.agents/ts-deliver-router/history.jsonl` (non-atomic). If the append fails, emit a warning and continue — history failure does NOT abort the phase exit.
...
history.jsonl append announced as `[DRY-RUN] would append .agents/ts-deliver-router/history.jsonl`; not executed.
