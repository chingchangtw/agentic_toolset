...
### [2026-07-05] Backgrounded bare `vitest` never exits — kills forgotten, not the process
- Context: Ran `npm test -- src/tests/unit/golden-templates.test.ts` in the background to check a new test file while doing other prep work.
- Mistake or friction: The `test` script is bare `vitest` with no `run` subcommand, which defaults to watch mode — it never exits on its own. The backgrounded process produced no visible output, so a foreground `npx vitest run <path>` was used instead to get a result, but the background watch process was never explicitly stopped. It sat orphaned, invisible, for the rest of the session.
- Missed signal: No output from a background task read shortly after launch was treated as "still starting up" rather than "this command doesn't terminate by design" — the two look identical from the output file alone.
- Better rule: Before backgrounding any test/dev command, check whether it's a one-shot or a watch/serve command. If a background task is abandoned in favor of a foreground rerun, stop the background one immediately rather than assuming it will exit on its own.
- Reusable fix: Prefer `npx vitest run <path>` (explicit one-shot) over the bare `npm test` script when backgrounding is intended, or don't background bare `vitest` at all.
- Should we create a script, template, test, or agent?: No — this is a general operating habit, not a repo-specific fix. Noted in personal/session memory as a standing rule (see feedback memory `feedback_stray_background_shells`).